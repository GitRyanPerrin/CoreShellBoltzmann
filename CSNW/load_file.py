...
def load_velocity():
    ...
    suffix = f"{ip.shape}_{ip.R}_{ip.t}_{ip.Nr}_{ip.Nphi}_{ip.alphaR}_{ip.betaD}"
    with np.load(f"./velocities/charge_velocities/cv_" + suffix + ".npz", allow_pickle=True) as file:
        v = file['arr_0']

    return v

def load_spin_velocity():

    suffix = f"{ip.shape}_{ip.R}_{ip.t}_{ip.Nr}_{ip.Nphi}_{ip.alphaR}_{ip.betaD}"
    with np.load(f"./velocities/spin_velocities/cv_" + suffix + ".npz", allow_pickle=True) as file:
        v = file['arr_0']
