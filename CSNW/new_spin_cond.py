...
def main():
    ...
    dFermi2 = np.stack([fermi_derivative(k, eigvals[:,:Nev], mu, T, order=2) for mu in chem_potential], axis=0)
    dFermi1 = np.stack([fermi_derivative(k, eigvals[:,:Nev], mu, T, order=1) for mu in chem_potential], axis=0)
    ...
    diffusive_integrand = np.stack([[diffusive_prod[ik, :Nev]*np.diag(dFermi2[imu, ik]) for ik in range(Nk)] for imu in range(Nmu)])
    ...
    e_prod = np.stack([v[ik]@v[ik] for ik in range(Nk)])
    e_integrand = np.stack([[e_prod[ik, :Nev]*np.diag(dFermi1[imu, ik]) for ik in range(Nk)] for imu in range(Nmu)])
