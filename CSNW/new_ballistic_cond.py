...
from load_file import load_eigensystem, load_velocity
...
def main():
    ...
    v = np.stack([eigvecs[ik].conj().T@eigvecs[ik]@v[ik] for ik in range(Nk)])
    ...
    #diffusive_prod = np.stack([eigvecs[ik].conj().T@v[ik]@eigvecs[ik]@eigvecs[ik].conj().T@v[ik]@eigvecs[ik] for ik in range(Nk)])
    #diffusive_prod = np.stack([v[ik]*v[ik] for ik in range(Nk)])
    #diffusive_prod = v[:,:Nev,:Nev]*v[:,:Nev, :Nev]
    diffusive_prod = v_diag*v_diag
