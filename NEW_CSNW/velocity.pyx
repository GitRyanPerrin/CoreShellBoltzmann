...
from velocity_matrix import Vmat
...
def calc_velocity_array(Nev, alphaR=ip.alphaR, betaD=ip.betaD, B=ip.B, save_file=False, Nk=2000, k=None):
    ...
    if save_file == True:
        np.savez_compressed(f"./velocities/charge_velocities/cv_{ip.shape}_{ip.R}_{ip.t}_{ip.Nr}_{ip.Nphi}_{ip.alphaR}_{ip.betaD}.npz", v)
    else:
        return v
...
def split_velocity_calculation(Nev, k, Nk):
    ...
    for d in range(dumps):
        ...
        np.savez_compressed(f"./velocities/charge_velocities/cv_{ip.shape}_{ip.R}_{ip.t}_{ip.Nr}_{ip.Nphi}_{ip.alphaR}_{ip.betaD}_({d}).npz", v)
