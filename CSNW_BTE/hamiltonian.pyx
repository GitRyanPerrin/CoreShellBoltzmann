...
def Hmat(k=ip.k, B=ip.B, n=0, E=ip.E, vertices=ip.vertices, t=ip.t, mag_dir = ip.mag_direction, alphaR = ip.alphaR, betaD = ip.betaD):
    ...
    for a1 in np.arange(1, Nstat+1, 1, dtype=int):
        ...
        if L == "inf":
            ...
            #H[a1-1,a1-1] = H[a1-1,a1-1] + ts*tk + 2*ts*tphi + E*r1/Rext
            ...
            if ip.SOI_definition == 'kokurin' or ip.SOI_definition == '111':
                ...
                if n1 == n2:
                    if (jj == 1 or jj == (Nphi - 1)):
                        ...
                        term1 = ts*1.0j*term1*betaD/4/sqrt(3)*sqrt(tphi)
